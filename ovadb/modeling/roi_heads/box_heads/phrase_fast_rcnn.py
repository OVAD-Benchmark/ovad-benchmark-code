...
from ovadb.modeling.utils import load_class_freq, get_fed_loss_inds

__all__ = ["PhraseFastRCNNOutputLayers"]


class PhraseFastRCNNOutputLayers(FastRCNNOutputLayers, LoggedModule):
    @configurable
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.sync_phrase_batch = sync_phrase_batch
        ...
        self.neg_phn_weight = neg_phn_weight
        ...
        ret.update(
            {
                ...
                "sync_phrase_batch": cfg.MODEL.SYNC_PHRASE_NOUNS_BATCH,
                "phrase_weight": cfg.MODEL.ROI_BOX_HEAD.PHRASE_NOUNS_WEIGHT,
                "neg_phn_weight": cfg.MODEL.ROI_BOX_HEAD.NEG_PHRASE_NOUNS_WEIGHT,
            }
        )
    ...
    def image_label_losses(
        ...
        num_inst_per_image = [len(p) for p in proposals]
        scores = predictions[0]
        cls_scores, caption_scores, phrase_scores = scores

        if cls_scores is not None:
            cls_scores = cls_scores.split(num_inst_per_image, dim=0)  # B x n x (C + 1)
        else:
            cls_scores = [None] * len(num_inst_per_image)

        if caption_scores is not None:
            if isinstance(caption_scores, tuple):
                caption_scores, neg_caption_scores = caption_scores
                neg_caption_scores = neg_caption_scores.split(num_inst_per_image, dim=0)
            else:
                neg_caption_scores = [None] * len(num_inst_per_image)
            caption_scores = caption_scores.split(
                num_inst_per_image, dim=0
            )  # B x n x (C + 1)
            caption_scores = [
                (cap, neg) for cap, neg in zip(caption_scores, neg_caption_scores)
            ]
        else:
            caption_scores = [None] * len(num_inst_per_image)

        if phrase_scores is not None:
            phrase_scores = phrase_scores.split(
                num_inst_per_image, dim=0
            )  # B x n x (C + 1)
        else:
            phrase_scores = [None] * len(num_inst_per_image)
        ...
        B = len(num_inst_per_image)
        # print("len(num_inst_per_image)", len(num_inst_per_image), "len(cls_scores)", len(cls_scores))

        loss = cls_scores[0].new_zeros([1])[0]
        caption_loss = cls_scores[0].new_zeros([1])[0]
        phrase_loss = cls_scores[0].new_zeros([1])[0]
        for idx, (
            cls_score,
            caption_score,
            phrase_score,
            ...
            if "caption" in ann_type:
                _, caption_loss_img = self._caption_loss(
                    caption_score, classifier_info, idx, B
                )
                caption_loss += self.caption_weight * caption_loss_img
                if ann_type == "caption":
                    continue
            if "phrase" in ann_type:
                phrase_loss_img = self._phrase_loss(
                    phrase_score, classifier_info, idx, B, p
                )
                phrase_loss += self.phrase_weight * phrase_loss_img
                if ann_type == "phrase":
                    continue
            if ("tag" in ann_type) or ("image" in ann_type):
                if self.debug:
                    p.selected = cls_score.new_zeros((len(p),), dtype=torch.long) - 1
                if cls_score is not None:
                    image_label_loss_img = self._image_label_loss(
                        cls_score, labels, classifier_info, idx, B, p, prop_score
                    )
                    loss += image_label_loss_img
        ...
        if "phrase" in ann_type:
            phrase_loss = phrase_loss / B
            loss = loss + phrase_loss
            storage.put_scalar("stats_l_phrase", phrase_loss.item())

        return {
            "image_loss": loss * self.image_loss_weight,
            "loss_cls": cls_score.new_zeros([1])[0],
            "loss_box_reg": cls_score.new_zeros([1])[0],
        }
    ...
    def _caption_loss(self, score, classifier_info, idx, B):
        ...
        assert (
            self.add_image_box
        ), "Using caption supervision required whole image representation. "
        if isinstance(score, tuple):
            caption_score, neg_score = score
        ...
        if neg_score is not None:
            ...
            caption_target = torch.cat(
                [caption_target, neg_score.new_zeros(neg_score.shape)], axis=1
            )
            caption_score = torch.cat([caption_score, neg_score], axis=1)
    ...
    def _phrase_loss(self, score, classifier_info, idx, B, p):
        assert classifier_info[3] is not None
        _, labels = classifier_info[3]
        labels = labels[idx]
        loss = score.new_zeros([1])[0]
        if self.image_label_loss == "max_score":
            for i_l, label in enumerate(labels):
                loss_i, ind = self._max_score_loss(score, label)
                loss += loss_i
        elif self.image_label_loss == "max_size":
            loss_i, ind = self._max_size_loss(score, labels, p)
            loss += loss_i
        elif self.image_label_loss == "first":  # max objectness
            loss_i, ind = self._first_loss(score, labels)
            loss += loss_i
        elif self.image_label_loss == "image":
            loss_i, ind = self._image_loss(score, labels)
            loss += loss_i
        elif self.image_label_loss == "min_loss":  # box with min loss
            for i_l, label in enumerate(labels):
    ...
    def _image_label_loss(self, score, labels, classifier_info, idx, B, p, prop_score):
        img_box_count = 0
        select_size_count = 0
        select_x_count = 0
        select_y_count = 0
    ...
    # Edited to include phrase nouns: classifier_info = (cls_features, cls_inds, caption_features, phn_features)
    def forward(self, x, classifier_info=(None, None, None, None)):
        """
        enable classifier_info
        """
        if x.dim() > 2:
            x = torch.flatten(x, start_dim=1)
        cls_scores, caption_scores, phrase_scores = None, None, None
        ...
        if classifier_info[2] is not None:
            ...
            neg_cap_cls = None
            if isinstance(cap_cls, tuple):
                assert not self.sync_caption_batch
                cap_cls, neg_cap_cls = cap_cls

            if self.sync_caption_batch:
                caption_scores = self.cls_score(x, classifier=cap_cls[:, :-1])
            else:
                caption_scores = self.cls_score(x, classifier=cap_cls)
                if neg_cap_cls is not None:
                    neg_caption_scores = self.cls_score(x, classifier=neg_cap_cls)
                    caption_scores = (caption_scores, neg_caption_scores)

        if classifier_info[3] is not None:
            phn_cls, _ = classifier_info[3]
            if self.sync_phrase_batch:
                phrase_scores = self.cls_score(x, classifier=phn_cls[:, :-1])
            else:
                phrase_scores = self.cls_score(x, classifier=phn_cls)

        scores = (cls_scores, caption_scores, phrase_scores)
        # scores = torch.cat(scores, dim=1) # B x C' or B x N or B x (C'+N)
        if classifier_info[2] is None and classifier_info[3] is None:
            scores = cls_scores  # B x C' or B x N or B x (C'+N)
