...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers, LoggedModule):
    @configurable
    def __init__(
        self,
    ...
    def from_config(cls, cfg, input_shape):
        ...
        if ret["use_zeroshot_cls"]:
            ...
            if cfg.MODEL.ROI_BOX_HEAD.WEIGHT_TRANSFER:
                ret["cls_score"] = WeightTransferZeroShotClassifier(cfg, input_shape)
            else:
                ret["cls_score"] = ZeroShotClassifier(cfg, input_shape)
        return ret
    ...
    def inference(self, predictions, proposals):
        ...
        # pred_instances, pred_idx = fast_rcnn_inference(
        #     boxes,
        #     scores,
        #     image_shapes,
        ...
        # features = predictions[0].split(num_inst_per_image, dim=0)
        # for img_ins, idx, feat, score in zip(
        #     pred_instances, pred_idx, features, scores
        # ):
        #     img_ins.feat = feat[idx]
        #     img_ins.score_dist = score[idx]

        # return pred_instances, pred_idx
    ...
    def inference_on_box(self, predictions, proposals):
        ...
        # scores, att_scores = self.predict_probs(predictions, proposals)
        # image_shapes = [x.image_size for x in proposals]
        # max_topk = int(max([len(b) for b in boxes]) * 1.1)
        # return fast_rcnn_att_inference_single_pred_per_box(
        #     boxes,
        #     scores,
        #     att_scores,
        #     image_shapes,
    ...
    def _max_size_loss(self, score, label, p):
        ...
        # print("loss print", label, ind, target)
        return loss, ind
