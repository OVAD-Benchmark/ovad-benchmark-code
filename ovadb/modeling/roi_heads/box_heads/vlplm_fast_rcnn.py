...
from .zero_shot_classifier import VLPLMZeroShotClassifier
...
class VLPLMFastRCNNOutputLayers(FastRCNNOutputLayers, LoggedModule):
    """
        ...
        ret.update(
            {
                ...
                "emb_dim": cfg.MODEL.ROI_BOX_HEAD.ZEROSHOT_WEIGHT_DIM,
                "loss_weight_background": 1.0,  # cfg.MODEL.ROI_BOX_HEAD.LOSS_WEIGHT_BACKGROUND,
            }
        )
        if ret["use_zeroshot_cls"]:
            ret["cls_score"] = VLPLMZeroShotClassifier(cfg, input_shape)
        return ret
    ...
    def set_class_embeddings(self, embs):
        ...
        self.cls_score.weight.data = torch.tensor(
            embs, device=device, requires_grad=False
        )
        self.cls_score.bias.data = torch.zeros_like(
            self.cls_score.bias.data, requires_grad=False
        )
    ...
    def losses(self, predictions, proposals):
        ...
        gt_confidence = (
            cat([p.gt_confidence for p in proposals], dim=0)
            if len(proposals)
            else torch.empty(0)
        )
    ...
    def box_reg_loss(
        self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, gt_confidence
    ):
        """
        Args:
        ...
        if self.box_reg_loss_type == "smooth_l1":
            ...
            loss_box_reg = (
                giou_loss(fg_pred_boxes, gt_boxes[fg_inds], reduction="none").sum(dim=1)
                * gt_confidence[fg_inds]
            ).sum()
        else:
...
def fast_rcnn_inference_single_image(
    ...
    # This next line is different than detectron2
