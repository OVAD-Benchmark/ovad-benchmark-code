...
class CustomCOCOEvaluator(COCOEvaluator):
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        """
        Additionally plot mAP for 'base classes' and 'novel classes'
        """
        ...
        novel_names = set([x["name"] for x in categories_novel17])
        extra_names = set(
            [x["name"] for x in COCO_CATEGORIES if x["name"] not in base_names]
        )
        ...
        results_per_category50_extended = []
        for idx, name in enumerate(class_names):
            ...
            if name in extra_names:
                results_per_category50_extended.append(float(ap50 * 100))
            ...
            + create_small_table(
                {
                    ...
                    "results_extended": np.nanmean(results_per_category50_extended),
                    "results_generalized": np.nanmean(
                        results_per_category50_base + results_per_category50_novel
                    ),
                    "results_generalized_extended": np.nanmean(
                        results_per_category50_base + results_per_category50_extended
                    ),
                }
            )
        )
        ...
        #     "base {} AP50: {}".format(
        #         iou_type,
        #         sum(results_per_category50_base) / len(results_per_category50_base),
        #     )
        # )
        # self._logger.info(
        #     "novel {} AP50: {}".format(
        #         iou_type,
        #         sum(results_per_category50_novel) / len(results_per_category50_novel),
        #     )
        # )
        ...
        results["AP50_17novel"] = sum(results_per_category50_novel) / len(
            results_per_category50_novel
        )
        results["AP50_32novel"] = sum(results_per_category50_extended) / len(
            results_per_category50_extended
        )
        return results
